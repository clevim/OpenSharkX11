use super::ConnectionMode;
use serde::{Deserialize, Serialize};

/// DPI lookup table: maps requested DPI → firmware byte value.
/// Sorted ascending; encode_dpi finds the smallest key >= requested dpi.
static DPI_MAP: &[(u32, u8)] = &[
    (50,0x01),(100,0x02),(150,0x03),(200,0x04),(250,0x05),(300,0x06),
    (350,0x08),(400,0x09),(450,0x0a),(500,0x0b),(550,0x0c),(600,0x0e),
    (650,0x0f),(700,0x10),(750,0x11),(800,0x12),(850,0x13),(900,0x15),
    (950,0x16),(1000,0x17),(1050,0x18),(1100,0x19),(1150,0x1b),(1200,0x1c),
    (1250,0x1d),(1300,0x1e),(1350,0x1f),(1400,0x20),(1450,0x22),(1500,0x23),
    (1550,0x24),(1600,0x25),(1650,0x26),(1700,0x27),(1750,0x29),(1800,0x2a),
    (1850,0x2b),(1900,0x2c),(1950,0x2d),(2000,0x2f),(2050,0x30),(2100,0x31),
    (2150,0x32),(2200,0x33),(2250,0x34),(2300,0x36),(2350,0x37),(2400,0x38),
    (2450,0x39),(2500,0x3a),(2550,0x3b),(2600,0x3d),(2650,0x3e),(2700,0x3f),
    (2750,0x40),(2800,0x41),(2850,0x43),(2900,0x44),(2950,0x45),(3000,0x46),
    (3050,0x47),(3100,0x48),(3150,0x4a),(3200,0x4b),(3250,0x4c),(3300,0x4d),
    (3350,0x4e),(3400,0x4f),(3450,0x51),(3500,0x52),(3550,0x53),(3600,0x54),
    (3650,0x55),(3700,0x57),(3750,0x58),(3800,0x59),(3850,0x5a),(3900,0x5b),
    (3950,0x5c),(4000,0x5e),(4050,0x5f),(4100,0x60),(4150,0x61),(4200,0x62),
    (4250,0x63),(4300,0x65),(4350,0x66),(4400,0x67),(4450,0x68),(4500,0x69),
    (4550,0x6b),(4600,0x6c),(4650,0x6d),(4700,0x6e),(4750,0x6f),(4800,0x70),
    (4850,0x72),(4900,0x73),(4950,0x74),(5000,0x75),(5050,0x76),(5100,0x77),
    (5150,0x79),(5200,0x7a),(5250,0x7b),(5300,0x7c),(5350,0x7d),(5400,0x7f),
    (5450,0x80),(5500,0x81),(5550,0x82),(5600,0x83),(5650,0x84),(5700,0x86),
    (5750,0x87),(5800,0x88),(5850,0x89),(5900,0x8a),(5950,0x8b),(6000,0x8d),
    (6050,0x8e),(6100,0x8f),(6150,0x90),(6200,0x91),(6250,0x93),(6300,0x94),
    (6350,0x95),(6400,0x96),(6450,0x97),(6500,0x98),(6550,0x9a),(6600,0x9b),
    (6650,0x9c),(6700,0x9d),(6750,0x9e),(6800,0x9f),(6850,0xa1),(6900,0xa2),
    (6950,0xa3),(7000,0xa4),(7050,0xa5),(7100,0xa7),(7150,0xa8),(7200,0xa9),
    (7250,0xaa),(7300,0xab),(7350,0xac),(7400,0xae),(7450,0xaf),(7500,0xb0),
    (7550,0xb1),(7600,0xb2),(7650,0xb3),(7700,0xb5),(7750,0xb6),(7800,0xb7),
    (7850,0xb8),(7900,0xb9),(7950,0xbb),(8000,0xbc),(8050,0xbd),(8100,0xbe),
    (8150,0xbf),(8200,0xc0),(8250,0xc2),(8300,0xc3),(8350,0xc4),(8400,0xc5),
    (8450,0xc6),(8500,0xc7),(8550,0xc9),(8600,0xca),(8650,0xcb),(8700,0xcc),
    (8750,0xcd),(8800,0xcf),(8850,0xd0),(8900,0xd1),(8950,0xd2),(9000,0xd3),
    (9050,0xd4),(9100,0xd6),(9150,0xd7),(9200,0xd8),(9250,0xd9),(9300,0xda),
    (9350,0xdb),(9400,0xdd),(9450,0xde),(9500,0xdf),(9550,0xe0),(9600,0xe1),
    (9650,0xe3),(9700,0xe4),(9750,0xe5),(9800,0xe6),(9850,0xe7),(9900,0xe8),
    (9950,0xea),(10000,0xeb),
    // 10100–12000: high-stage range (byte 16-21 set to 0x01 by build_dpi)
    (10100,0x76),(10200,0x77),(10300,0x79),(10400,0x7a),(10500,0x7b),(10600,0x7c),
    (10700,0x7d),(10800,0x7f),(10900,0x80),(11000,0x81),(11100,0x82),(11200,0x83),
    (11300,0x84),(11400,0x86),(11500,0x87),(11600,0x88),(11700,0x89),(11800,0x8a),
    (11900,0x8b),(12000,0x8d),(12100,0x8e),(12200,0x8f),(12300,0x90),(12400,0x91),
    (12500,0x93),(12600,0x94),(12700,0x95),(12800,0x96),(12900,0x97),(13000,0x98),
    (13100,0x9a),(13200,0x9b),(13300,0x9c),(13400,0x9d),(13500,0x9e),(13600,0x9f),
    (13700,0xa1),(13800,0xa2),(13900,0xa3),(14000,0xa4),(14100,0xa5),(14200,0xa7),
    (14300,0xa8),(14400,0xa9),(14500,0xaa),(14600,0xab),(14700,0xac),(14800,0xae),
    (14900,0xaf),(15000,0xb0),(15100,0xb1),(15200,0xb2),(15300,0xb3),(15400,0xb5),
    (15500,0xb6),(15600,0xb7),(15700,0xb8),(15800,0xb9),(15900,0xbb),(16000,0xbc),
    (16100,0xbd),(16200,0xbe),(16300,0xbf),(16400,0xc0),(16500,0xc2),(16600,0xc3),
    (16700,0xc4),(16800,0xc5),(16900,0xc6),(17000,0xc7),(17100,0xc9),(17200,0xca),
    (17300,0xcb),(17400,0xcc),(17500,0xcd),(17600,0xcf),(17700,0xd0),(17800,0xd1),
    (17900,0xd2),(18000,0xd3),(18100,0xd4),(18200,0xd6),(18300,0xd7),(18400,0xd8),
    (18500,0xd9),(18600,0xda),(18700,0xdb),(18800,0xdd),(18900,0xde),(19000,0xdf),
    (19100,0xe0),(19200,0xe1),(19300,0xe3),(19400,0xe4),(19500,0xe5),(19600,0xe6),
    (19700,0xe7),(19800,0xe8),(19900,0xea),(20000,0xeb),
    // 20100–22000: highest range (bytes 16-21 set to 0x01)
    (20100,0xeb),(20200,0x76),(20300,0x76),(20400,0x77),(20500,0x77),(20600,0x79),
    (20700,0x79),(20800,0x7a),(20900,0x7a),(21000,0x7b),(21100,0x7b),(21200,0x7c),
    (21300,0x7c),(21400,0x7d),(21500,0x7d),(21600,0x7f),(21700,0x7f),(21800,0x80),
    (21900,0x80),(22000,0x81),
];

fn encode_dpi(dpi: u32) -> Option<u8> {
    DPI_MAP.iter().find(|(k, _)| *k >= dpi).map(|(_, v)| *v)
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RgbColor {
    pub r: u8,
    pub g: u8,
    pub b: u8,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[repr(u8)]
pub enum LedMode {
    Off      = 0x00,
    Blink    = 0x01,
    Flash    = 0x02,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DpiConfig {
    pub dpi_values: [u32; 6],
    pub active_stage: u8, // 1-6
    pub angle_snap: bool,
    pub ripple_control: bool,
    pub stage_colors: [RgbColor; 6],
    pub led_mode: LedMode,
}

impl Default for DpiConfig {
    fn default() -> Self {
        Self {
            dpi_values: [800, 1600, 2400, 3200, 5000, 22000],
            active_stage: 2,
            angle_snap: false,
            ripple_control: true,
            stage_colors: [
                RgbColor { r: 0xff, g: 0x00, b: 0x00 },
                RgbColor { r: 0x00, g: 0xff, b: 0x00 },
                RgbColor { r: 0x00, g: 0x00, b: 0xff },
                RgbColor { r: 0xff, g: 0xff, b: 0x00 },
                RgbColor { r: 0x00, g: 0xff, b: 0xff },
                RgbColor { r: 0xff, g: 0x00, b: 0xff },
            ],
            led_mode: LedMode::Off,
        }
    }
}

pub fn build_dpi(cfg: &DpiConfig, mode: ConnectionMode) -> Vec<u8> {
    let mut buf = [0u8; 56];

    buf[0] = 0x04; buf[1] = 0x38; buf[2] = 0x01;
    buf[3] = if cfg.angle_snap    { 0x01 } else { 0x00 };
    buf[4] = if cfg.ripple_control { 0x01 } else { 0x00 };
    buf[5] = 0x3f;

    // DPI stage values (bytes 8-13)
    for (i, &dpi) in cfg.dpi_values.iter().enumerate() {
        buf[8 + i] = encode_dpi(dpi).unwrap_or(0x12); // fallback to 800
    }

    // Active stage
    buf[24] = cfg.active_stage.clamp(1, 6);

    // Stage colors (bytes 25-48, 3 bytes per stage)
    for (i, color) in cfg.stage_colors.iter().enumerate() {
        let off = 25 + i * 3;
        buf[off]     = color.r;
        buf[off + 1] = color.g;
        buf[off + 2] = color.b;
    }

    buf[49] = cfg.led_mode as u8;

    // Stage mask: bit set if DPI > 12000
    let mut mask = 0u8;
    for (i, &dpi) in cfg.dpi_values.iter().enumerate() {
        if dpi > 12000 { mask |= 1 << i; }
    }
    buf[6] = mask;
    buf[7] = mask;

    // High-stage flags (bytes 16-21): set to 0x01 for ranges [10100-12000] or [20100-22000]
    for (i, &dpi) in cfg.dpi_values.iter().enumerate() {
        buf[16 + i] = if (dpi >= 10100 && dpi <= 12000) || (dpi >= 20100 && dpi <= 22000) {
            0x01
        } else {
            0x00
        };
    }

    // Checksum: sum of bytes 3..=49, 16-bit big-endian at bytes 50-51
    let checksum: u16 = buf[3..=49].iter().map(|&b| b as u16).sum();
    buf[50] = (checksum >> 8) as u8;
    buf[51] = (checksum & 0xff) as u8;

    match mode {
        ConnectionMode::Wired    => buf[..52].to_vec(),
        ConnectionMode::Wireless => buf.to_vec(),
    }
}

pub const DPI_CONTROL_PARAMS: (u8, u8, u16, u16) = (0x21, 0x09, 0x0304, 2);
